...
#Created 2025-01-30 at 11:51

xpose.runno <- ''
toolname <- 'execute'
pdf.filename <- paste0('PsN_',toolname,'_plots.pdf')
working.directory<-'/Users/jeff/Desktop/nonmem_project/experiments/exp1/modelfit_dir4/'
results.directory <- '..'
subset.variable<-NULL
tab.suffix <- '' 
rscripts.directory <- '/Library/Perl/5.34/PsN_5_5_0/R-scripts' # This is not used
raw.results.file <- 'raw_results_2cmt_oral.csv'
theta.labels <- c('1 prop','2 add','3 CL','4 V2','5 V3','6 Q','7 KA')
theta.fixed <- c(FALSE,FALSE,FALSE,FALSE,FALSE,FALSE,FALSE)
omega.labels <- c('','_')
omega.fixed <- c(FALSE,FALSE)
sigma.labels <- c('residual variability')
sigma.fixed <- c(TRUE)
n.eta <- 2
n.eps <- 1
